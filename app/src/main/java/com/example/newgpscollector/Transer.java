...
public class Transer {
    ...
    private static LatLng WGS2GCJ02(double wgLat, double wgLon){
        LatLng point=null;
    ...
    private static LatLng GCJ02ToBD09(double gg_lat, double gg_lon)
    {
    ...
    public static LatLng WGS2BD09(double lat,double lon){
        LatLng point=WGS2GCJ02(lat,lon);
        point=GCJ02ToBD09(point.latitude, point.longitude);
        if(!inChina(lat,lon)){
            point=new LatLng(lat,lon);
        }//不在国内，按GPS输出
        return point;
    ...
    public static LatLng XY2BL(double x,double y,double n){
        double b=a-a*f;
        double e=(a*a-b*b)/(a*a);//计算第一偏心率
        double m_e=(a*a-b*b)/(b*b);//计算第二偏心率
    ...
    public static LatLng baiduToWgs84(double bdLat,double bdLon){
        double PI = 3.14159265358979324;
        ...
        dLat = gcjLat - dLat;
        dLon = gcjLon - dLon;
        LatLng res=new LatLng(dLat,dLon);
        return res;
    }
}
